	...
	<div class="articleld">
		...
		<header>
			<div class="header">
				...
				<div class="right">
					<i class="iconfont">&#xe69d;</i>
					<img :src="require('@/static/article/author.jpg')" alt="作者简介" @click="">
				</div>
			</div>
		</header>
		<div class="container">
			...
			<div class="timer stuff">
				<span>{{dataContent.timer | getTimer}}</span>
				<span>字数 2412</span>
				<span v-for="(item,index) in dataContent.infoIcon" :key="index">{{item.text}} {{item.num}}</span>
			</div>
			<!-- <div style="height: 1000px;"></div> -->
			<div class="content" v-highlight v-html="dataContent.container" id="content">
			</div>
		</div>
	...
	export default {
		...
		mounted() {
			this.$nextTick(() => {
				...
				this.options.forEach((o, i) => {
					if (o.type == 'comment') {
						o.num = this.dataContent.infoIcon[0].num;
					} else if (o.type == 'read') {
						o.num = this.dataContent.infoIcon[1].num;
					} else if (o.type == 'like') {
						o.num = this.dataContent.infoIcon[2].num;
					}
				})
			})
		},
		...
		methods: {
			getOne() {
				// const {
				// 	data: res
				// } = await getByIdApi(this.i);
				// this.dataContent = res.data;
				this.content.forEach((o, i) => {
					if (o.id == this.i) {
						this.dataContent = o;
					}
				})
			...
			getHeight() {
				const domList = ['.stuff', '.title','.content']
