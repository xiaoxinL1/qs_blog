	...
	<div id="app">
		...
		<div class="index">
			<div class="cover">
				...
				<div id="bgimg" :style="layerStyle">
					...
					<img id="image" width="1920" height="1080" :style="imgStyle" :src="img" />
				</div>
				<!-- 遮罩层 -->
				<div class="mask">
				</div>
		...
		<div class="content">
			<div class="card" v-for="(item,index) in content" :key="index"
				:class="index%2==0?'wow animate__fadeInRight ':'wow animate__fadeInLeft'" data-wow-duration="1s">
				<router-link :to="'/article?id='+item.id">
					<div class="img-box">
						<img v-lazy="item.image" :key="item.image" />
					</div>
					<div class="info">
						<div class="time">{{item.timer | getTimer}}</div>
						<div class="title">
							<a>{{item.title}}</a>
						</div>
						<div class="describe">{{item.content}}</div>
						<div class="stuff">
							<div v-for="(v, i) in item.infoIcon" :key="i">
...
<script>
	...
	export default {
		...
		mounted() {
			...
			const src = require('@/static/index/bg.jpg')
			this.getBase64(src).then(res => {
				this.img = res;
			});
			this.$nextTick(()=>{
				this.$wow.init()
